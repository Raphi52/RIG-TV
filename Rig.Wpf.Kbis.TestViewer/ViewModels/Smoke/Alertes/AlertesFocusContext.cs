...
// FocusContext ALERTES RCS — miroir de KbisFocusContext (2026-05-29).
...
public sealed class AlertesFocusContext : IFocusContext
{
    ...
    public AlertesFocusContext(AlertesScenarioCatalog catalog)
    {
        _catalog = catalog;
        ...
        OpenHdeskCommand = new RelayCommand(() =>
        {
            ...
            {
                Log.Warn("AlertesFocusContext.OpenHdeskCommand — pas de DesktopName (worker mort ou pas démarré)");
                return;
            }
            Log.Info($"AlertesFocusContext.OpenHdeskCommand — SwitchToHDesk('{desk}')");
        ...
        CloseHdeskCommand = new RelayCommand(() =>
        {
            ...
            if (SelectedItem?.WorkerPid is int pid)
            {
                try
                {
                    ...
                    Log.Info($"AlertesFocusContext.CloseHdeskCommand — taskkill /T /F /PID {pid}");
                }
    ...
    public ICommand OpenSnapsRootCommand { get; } = new RelayCommand(() =>
        Log.Warn("AlertesFocusContext.OpenSnapsRootCommand — pas implémenté"));
    public ICommand OpenSnapsFolderCommand { get; } = new RelayCommand(() =>
        Log.Warn("AlertesFocusContext.OpenSnapsFolderCommand — pas implémenté"));
    public ICommand CopyLogsCommand { get; } = new RelayCommand(() =>
        Log.Warn("AlertesFocusContext.CopyLogsCommand — pas implémenté"));
