...
// Adapter ScenarioViewModel (modèle legacy smoke) → IScenarioItem, module ALERTES RCS.
// 2026-05-29 — miroir instance-based de KbisLegacyScenarioAdapter (cf. Smoke\Kbis\).
// Une instance = UNE tuile parallèle d'un scénario Alertes (int-form / int-dca / rec-form / rec-dca).
// L'instanceId (= workerId = sous-dossier self-snap) route les lignes stdout + les PNG.
// Phase/Verdict/Steps calculés PAR INSTANCE depuis les lignes filtrées par instanceId.
...
/// <summary>
/// Wrappe UNE instance d'un scénario Alertes RCS pour MosaicView / FocusView / ScenarioListView
/// via <see cref="IScenarioItem"/>. Routing par <see cref="_instanceId"/> (ex "alertes-int-form-1").
/// </summary>
public sealed class AlertesLegacyScenarioAdapter : INotifyPropertyChanged, IScenarioItem
{
    ...
    private readonly string _kind;       // "int-form" | "int-dca" | "rec-form" | "rec-dca"
    private readonly bool _showSuffix;

    public event PropertyChangedEventHandler? PropertyChanged;

    public AlertesLegacyScenarioAdapter(
    ...
    /// <summary>Discriminant du scénario : "int-form" / "int-dca" / "rec-form" / "rec-dca".
    /// Utilisé par PlayAlertesScenario pour relancer CE scénario seul.</summary>
    public string Kind => _kind;

    /// <summary>Identité unique (workerId + sous-dossier self-snap).</summary>
    public string InstanceId => _instanceId;

    // ── Steps attendus par kind ─────────────────────────────────────────────
    // Étape 1 (scaffolding) : Sanity + Lancement + Login + Ouvrir alerte RCS (terminal).
    // Les steps métier (trouver demande, double-clic, DocDemat, réclamation) seront ajoutés
    // en Étape 3 quand le driver SmokeRunner les émettra.
    ...
    private static readonly Dictionary<string, List<(string prefix, string label)>> ExpectedByKind = new()
    {
    ...
    private List<(string prefix, string label)> ExpectedSteps =>
        ExpectedByKind.TryGetValue(_kind, out var l) ? l : Common;
